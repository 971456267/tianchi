...
def cross_feature_two(data):
    ...
    #统计用户点击个多少种shop_review_num_level
    """
...
def cross_feature_two_one_day(data):
    ...
    """
    #统计用户点击个多少种item_city_id
    user_city= data.groupby(['user_id','day','item_city_id'],as_index = False)
    show = user_city['is_trade'].agg([np.size])
    show = show.reset_index()
    show.columns = ['user_id','day','item_city_id','user_day_city']
    data = pd.merge(data,show,on=['user_id','day','item_city_id'],how = 'left')
    #统计用户点击个多少种item_sales_level
    user_sales= data.groupby(['user_id','day','item_sales_level'],as_index = False)
    show = user_sales['is_trade'].agg([np.size])
    show = show.reset_index()
    show.columns = ['user_id','day','item_sales_level','user_day_sales']
    data = pd.merge(data,show,on=['user_id','day','item_sales_level'],how = 'left')
    #统计用户点击个多少种item_collected_level
    user_collected= data.groupby(['user_id','day','item_collected_level'],as_index = False)
    show = user_collected['is_trade'].agg([np.size])
    show = show.reset_index()
    show.columns = ['user_id','day','item_collected_level','user_day_collected']
    data = pd.merge(data,show,on=['user_id','day','item_collected_level'],how = 'left')
    #统计用户点击个多少种item_price_leve
    user_price= data.groupby(['user_id','day','item_price_level'],as_index = False)
    show = user_price['is_trade'].agg([np.size])
    show = show.reset_index()
    show.columns = ['user_id','day','item_price_level','user_day_price']
    data = pd.merge(data,show,on=['user_id','day','item_price_level'],how = 'left')
    #统计用户点击个多少种item_collected_level
    user_pv= data.groupby(['user_id','day','item_pv_level'],as_index = False)
    show = user_pv['is_trade'].agg([np.size])
    show = show.reset_index()
    show.columns = ['user_id','day','item_pv_level','user_day_pv']
    data = pd.merge(data,show,on=['user_id','day','item_pv_level'],how = 'left')
    """
    return data
     ...
     if True:
        ...
        #容易过拟合的特征
         ...
         if online == False:
             ...
             train[['instance_id','is_trade', 'predicted_score']].to_csv('new_lgb_train_baseline.csv', index=False,sep=' ')
             test[['instance_id', 'predicted_score']].to_csv('new_lgb_test_baseline.csv', index=False,sep=' ')
